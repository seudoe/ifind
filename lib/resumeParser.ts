...
async function parseWithHuggingFace(pdfBuffer: Buffer): Promise<ParsedResumeData> {
...
export async function parseResumeWithAI(pdfBuffer: Buffer): Promise<ParsedResumeData> {
  // 1. HuggingFace Space
  try {
    console.log(`[resumeParser] Trying HuggingFace Space (timeout: ${MAX_WAIT_FOR_HF_MS / 1000}s)...`);
  ...
  // All failed
  console.error("[resumeParser] All 3 resume parser providers (HF -> OpenAI -> Gemini) failed.");
  throw new Error("Unable to extract resume data at this moment. Please try again later.");
}
